...
# -*- coding: utf-8 -*-
"""
Created on Fri Aug 17 09:50:36 2018
...
import os
import sys
input_path = "D:/Python/foundations practice/excel"
output_file = "D:/Python/foundations practice/excel/14output_pandas.xls"
...
for workbook in all_workbooks:
	...
	for worksheet_name, cell in all_worksheets.items():
		total_sales = pd.DataFrame([float(str(value).strip('$').replace(',','')) \
                      for value in cell.loc[:, 'Sale Amount']]).sum()
		number_of_sales = len(cell.loc[:, 'Sale Amount'])
	...
	workbook_stats = pd.DataFrame(workbook_stats, \
                               columns=['workbook', 'workbook_total', \
                                        'workbook_average'])
	workbook_data_frame = pd.merge(worksheets_data_frame, \
                                workbook_stats, on='workbook', how='left')
